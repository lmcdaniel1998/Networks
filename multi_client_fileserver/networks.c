...
int udpForkSetup(struct sockaddr_in6* forkInfo) {
	int socketNum = 0;
	int forkAddrLen = sizeof(struct sockaddr_in6);
	
	// create fork socket
	if ((socketNum = socket(AF_INET6, SOCK_DGRAM, 0)) < 0) {
		perror("fork socket() error");
		return -1;
	}

	// set up the socket
	forkInfo->sin6_family = AF_INET6;		// internet (IPv6 or IPv4) family
	forkInfo->sin6_addr = in6addr_any;				// use any local IP address
	forkInfo->sin6_port = 0;			// pick one with bind

	// bind address to a port
	if (bind(socketNum, (struct sockaddr *) forkInfo, forkAddrLen) < 0)
	{
		perror("fork bind() error");
		return -1;
	}

	// get the port number */
	getsockname(socketNum, (struct sockaddr *) forkInfo, (socklen_t *) &forkAddrLen);
	printf("Fork using Port #: %d\n", ntohs(forkInfo->sin6_port));
